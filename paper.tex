...
Let $p \in \reals^{n+1}$, let $S$ be a subset of the indices of $p$, and let $B$ be the matrix from~\eqref{eq:matrxBobtusebasis} with column given by the superbasis vectors $b_1,\dots,b_{n+1}$.  The following equalities hold:
\begin{enumerate}
...
\]
since $\sum_{i=1}^{n+1} q_{ij} = 0$ as a result of the superbasis condition~\eqref{eq:superbasecond}.  Now
\begin{align*}
