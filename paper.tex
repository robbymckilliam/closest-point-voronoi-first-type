...
%BLERG.  THIS MIGHT BETTER GO IN THE GRAPH SECTION
% There is a one-to-one correspondence between the obtuse superbasis of a lattice of Vornoi's first kind and what is called the \emph{Laplacian} of a weighted graph.  To se this, let
% \[
% B = (b_1\,\,b_2\,\,\dots\,\,b_{n+1})
% \]
% be the matrix with columns given by $b_1,\dots,b_{n+1}$ and define the $n+1$ by $n+1$ \emph{Gram matrix} $Q = B^\prime B$.  The elements of $Q$ are given by the Selling parameters, that is, $Q_{ij} = q_{ij}$.  Let $G$ be a graph with $n+1$ vertices $v_1,\dots,v_{n+1}$ and edges $e_{ij}$ connecting $v_i$ to $v_j$.  To each edge assign the \emph{weight} $w_{ij} = -q_{ij} \leq 0$.  Let $d_{ii} = \sum_{j \neq i} w_{ij}$ be the sum of weights of edges connected to vertex $v_i$. The Laplacian of the graph $G$ is the $n+1$ by $n+1$ matrix with elements
% \[
% L_{ij} = \begin{cases}
% d_{ii} & i = j \\
% -w_{ij} & i \neq j. 
