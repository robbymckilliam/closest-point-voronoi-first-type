...
\end{lemma}
\begin{proof}
The lemma is trivial if $\subrng(v) = \emptyset$ so that $\decrng(v) = v$.  It remains to prove the lemma when $\subrng(v) \neq \emptyset$.  In this case put $S = \subrng(v)$ and put $u = \decrng(v) = v - \onebf_S$.  Let $\zeta = z - \floor{z}$ be the column vector containing the fractional parts of the elements of $z$.  We have $\zeta - u = \zeta - v + \onebf_S$.  Applying part~\ref{eq:lem:decSellingsinc} of Lemma~\ref{lem:decSellings} with $p = \zeta - v$ we obtain
\begin{align}
...
h_i - h_j \geq v_i - v_j
\]
when either $v_i = \min(v)$ or $v_j = \max(v)$.
\end{lemma}
\begin{proof}
If $v_i = \min(v)$ then $h_i = 0$ and we need only show that $-h_j \geq \min(v) - v_j$ for all $j$.  If $v_j = \min(v)$ then $h_j = 0 = \min(v) - v_j$ and the results holds.  Otherwise if $v_j > \min(v)$ then $-h_j \geq -1 \geq \min(v) - v_j$ because $\min(v)$ and $v_j$ are integers.

Now, if $v_j = \max(v)$ then $h_j = 1$ and we need only show that $h_i - 1 \geq v_i - \max(v)$ for all $i$.  If $v_i = \max(v)$ then $h_i = 1$ and $h_i - 1 = 0 = v_i - \max(v)$.  Otherwise if $v_i < \max(v)$ then $h_i - 1 \geq -1 \geq v_i - \max(v)$.
\end{proof}
...
As stated already $h_i = 0$ whenever $v_i = \min(v)$ and $h_i = 1$ whenever $v_i = \max(v)$.  It follows by Lemma~\ref{eq:integergreaterless} that
\begin{equation}\label{eq:hijvijieq}
h_{i} - h_j \geq v_{i} - v_j
\end{equation}
when either $v_i = \min(v)$ or $v_j = \max(v)$.  Since $v_i = \min(v)$ for $i \in T$ and $v_j = \max(v)$ for $j \in S$ the inequality~\eqref{eq:hijvijieq} holds whenever $i \in T$ or $j \in S$.

Put $r = z - u - h$.  By~\eqref{eq:hijvijieq} we have $r_i - r_j \leq p_{i} - p_{j}$ when either $i \in T$ or $j \in S$.  Now, since $q_{ij} \leq 0$ for $i \neq j$,
\[
\Phi(S,r) = \sum_{i \in S}\sum_{j \notin S}q_{ij}(1 + 2r_i - 2r_j) \geq \sum_{i \in S}\sum_{j \notin S}q_{ij}(1 + 2p_i - 2p_j) = \Phi(S,p).
\]
Similarly
\[
\Phi(\bar{T},r) = \sum_{i \notin T}\sum_{j \in T}q_{ij}(1 + 2r_i - 2r_j) \geq \sum_{i \notin T}\sum_{j \in T}q_{ij}(1 + 2p_i - 2p_j) = \Phi(\bar{T},p).
\]\
Now
\begin{align*}
