...
Let $p \in \reals^{n+1}$, let $S$ and $T$ be subsets of the indices of $p$, and let $B$ be the matrix from~\eqref{eq:matrxBobtusebasis} with columns given by the superbasis vectors $b_1,\dots,b_{n+1}$.  The following equalities hold:
\begin{enumerate}
...
% \item  \label{eq:lem:decSellingsincanddec} \begin{align*}
% \|Bp\|^2 - \|B(p + I_S + I_T)\|^2 = \sum_{i \in S}\sum_{j \notin S}q_{ij}(1 + 2p_j - 2p_i) + \sum_{i \notin T}\sum_{j \in T}q_{ij}(1 + 2p_j - 2p_i) + \sum_{i\in S}\sum_{j\in S} q_{ij}, 
% \end{align*}
\end{enumerate}
...
\]
and put $d = g - I_A + I_D \in \{0,1\}^{n+1}$ and $w = v - d$.  Observe that $d_i = 0$ whenever $v_i = \min(v)$ and so $\min(w) = \min(v-d) = \min(v)$.  Also, $d_i = 1$ whenever $v_i = \max(v)$ and so $\max(w) = \max(v-d) = \max(v) - 1$.  Thus,
\[
\rng(w) = \max(v) - 1 - \min(v) = \rng(v) - 1 = \ell - 1.
\]
The lemma will follow if we show that $B(u+g+w)$ is a closest lattice point to $y$ since then the lattice point $B(u+g)$ with be $(\ell-1)$-close to $y$.  We show this in two stages.  First put $f = g - I_A \in \{0,1\}^{n+1}$ and  $h = v - f = v - g + I_A$.  Suppose that $B(u+g+h)$ is not a closest lattice point to $y = Bz$.  That is, with $t = z-u$, suppose that
\[
\|B(t-g-h)\|^2 = \|B(t-v-I_A)\|^2 > \|B(t-v)\|^2.
\]
By part~\ref{eq:lem:decSellingsdec} of Lemma~\ref{lem:decSellings} we obtain the inequality
\[
\|B(t-v)\|^2 - \|B(t-v-I_A)\|^2 =  \sum_{i \in A}\sum_{j \notin A}q_{ij}(1 + 2(t_j-t_i) + 2(v_i-v_j)) < 0.
\]
Because $v_i = \min(v)$ only when $f_i= g_i - 1 = 0$ and because $v_i = \min(v)$ for $i \in A$ we have $f_i - f_j \geq v_i - v_j$ for all $i \in S$ and $j =1,\dots,n+1$.  Now, since $q_{ij} \leq 0$ for $i\neq j$,
\begin{align*}
\|B(z-u-f)\|^2 - \|B(z-u-g)\|^2 &= \|B(t-f)\|^2 - \|B(t-f-I_A)\|^2 \\
&= \sum_{i \in A}\sum_{j \notin A}q_{ij}(1 + 2(t_j-t_i) + 2(f_i-f_j)) \\
&\leq  \sum_{i \in A}\sum_{j \notin A}q_{ij}(1 + 2(t_j-t_i) + 2(v_i-v_j)) < 0,
\end{align*}
contradicting~\eqref{eq:qismin} because $f \in \{0,1\}^{n+1}$.  Thus, $B(u+g+h)$ is a closest lattice point to $y = Bz$.

Now suppose that $B(u+g+w) = B(u+g+h-I_D)$ is not a closest lattice point to $y$.  That is, suppose that
\[
\|B(t-g-w)\|^2 = \|B(t-g-h+I_D)\|^2 > \|B(t-g-h)\|^2.
\]
By part~\ref{eq:lem:decSellingsinc} of Lemma~\ref{lem:decSellings} we obtain the inequality
\[
\|B(t-v)\|^2 - \|B(t-v-I_A)\|^2 =  \sum_{i \in A}\sum_{j \notin A}q_{ij}(1 + 2(t_j-t_i) + 2(v_i-v_j)) < 0.
\]
...
% \]
% Lemma~\ref{lem:vqcontramax} (proved below) with $t = z-u$ and $S = D$ shows that~\eqref{eq:lem:rng:contag} occurs again, contradicting~\eqref{eq:qismin}.  

Thus, $B(u+g+w)$ is a closest lattice point to $y$ and, since $\rng(w) = \ell-1$, the lattice point $B(u+g)$ is $(\ell-1)$-close to $y$.
 \end{proof}
