...
\\\textbf{Response:}
The final paragraph of Section~\zref{sec:discussion} has been modified to make the connection with the existing problem of determining whether a rectangular basis exists.
...
Among the iterative proceedures in our paper, in~\cite{Shalvi_iterativeslicer_2009}, and in~\cite{MicciancioVoulgaris_deterministic_jv_2013}, ours is the simplest.  The choice of our iteration is not motivated by what might be fastest in general lattices.  It is motivated by what we can prove to be polynomial time for those lattices of Vornoi's first kind.
...
The iterative algorithm of Micciancio and Voulgaris~\cite{MicciancioVoulgaris_deterministic_jv_2013} is different again.  To our (admittedly rough) knowledge, the proceedure computes an $\alpha_k$ (different, but related to that above) and then proceeds by the same iteration as ours to compute a closest point in the scaled lattice $\alpha_k \Lambda$.  The next $\alpha_{k+1}$ is computed and the proceedure repeats.

We beleive that detailing the differences between these iterative proceedures would only detract from the core purpose of our paper to decscribe an $O(n^4)$ time algorithm to compute a closest point in lattice of Vornoi's first kind.
...
\\\textbf{Response:}
We thank the reviewer for these interesting references highlighting the difficultly of computing close approximations of a closest lattice point.  These have been referenced in Section~\zref{sec:iterative-slicer} with the following text:
