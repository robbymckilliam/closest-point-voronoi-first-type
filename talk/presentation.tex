...
\begin{align*}
x_{k+1} &= x_k + v_k \\
v_k &= \arg\min_{ v \in \relevant(\Lambda) \cup \{\zerobf\} } \|y - x_k - v \|
\end{align*} 
}
...
\[
\arg\min_{ v \in \relevant(\Lambda) \cup \{\zerobf\} } \|y - x_k - v \|
\]
might be expensive.
...
\begin{itemize}
\item The intial point $x_0$ can be chosen to ensure that at most $K = n$ iterations is required before a closest point in found.
\item Minimisation over the set of relevant vector can be performed by computing a minimimum cut in a flow network.
...
\frame{
\frametitle{A series of relevant vectors}
{\color{gray}For a lattice of Voronoi's first kind:}
\begin{itemize}
\item The intial point $x_0$ can be chosen to ensure that at most $K = n$ iterations is required before a closest point in found.
\item {\color{gray}Minimisation over the set of relevant vector can be performed by computing a minimimum cut in a flow network.}
\item {\color{gray}Using known algorithms a minimum cut can be found in $O(n^3)$ operations.}
\item {\color{gray}In total $O(n^4)$ operations are required to compute a closest lattice point.}
\end{itemize}
}
...
\item Do applications, such as global position, phase unwrapping, circular statistics, involve lattices of Voronoi's first kind?
\end{itemize}
